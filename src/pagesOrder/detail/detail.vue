...
import { deleteMemberOrderAPI, getMemberOrderByIdAPI, getMemberOrderConsignmentByIdAPI, getMemberOrderLogisticsByIdAPI, putMemberOrderReceiptByIdAPI } from '@/services/order'
...
<template>
  ...
  <scroll-view scroll-y class="viewport" id="scroller" enable-back-to-top @scrolltolower="onScrolltolower">
    <template v-if="order">
      <!-- 订单状态 -->
  ...
  <view v-if="order.orderState === OrderState.DaiShouHuo" @tap="onOrderConfirm" class="button">
    确认收货
      ...
      <view class="goods">
        <view class="item">
          <navigator class="navigator" v-for="item in order.skus" :key="item.id" :url="`/pages/goods/goods?id=${item.spuId}`"
            hover-class="none">
